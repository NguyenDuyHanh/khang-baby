...
async function fixNames() {
  try {
    const { rows: products } = await pool.query("SELECT p.id, p.ten_sp, c.ten_danh_muc FROM hang_hoa p JOIN danh_muc c ON p.id_danh_muc = c.id WHERE p.ten_sp LIKE 'Hàng hóa thực tế%'");
    
    let totalUpdated = 0;
    for (const p of products) {
      const catName = p.ten_danh_muc;
      const targetList = productData[catName];
      
      let newName = "Sản phẩm thực tế";
      if (targetList && targetList.length > 0) {
        // Randomly select one from the list and append a suffix to make it slightly different
        const baseName = targetList[Math.floor(Math.random() * targetList.length)];
        const suffixes = [" (Mẫu mới)", " (Nhập khẩu)", " (Chính hãng)", " (Loại to)"];
        newName = baseName + suffixes[Math.floor(Math.random() * suffixes.length)];
      } else {
        // Fallback for categories not in user's list (e.g. Sách & Truyện tranh)
        newName = "Sản phẩm thực tế - " + catName + " (Chính hãng)";
      }
    ...
    console.log(`Successfully fixed ${totalUpdated} remaining products.`);
  } catch (error) {
